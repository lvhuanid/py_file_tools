    ...
    for file_name in file_names:
        ...
        if file_name.endswith('.java'):
            ...
            with open(file_path, 'r') as file:
                ...
                # 处理加载的JSON数据，这里只是简单打印
                # print(json_data)
    with open(f'new_java/1.json', 'w') as j_file:
...
def test(file_path, datas):
    ...
    for match in matches_init:
        if match.group(1) == "OnuVPortSvcProfTable":
            contents.append("VirtualPortServiceProfile")
        elif match.group(1) == "OnuRateControlSchedulerProfTable":
            contents.append("RateControlProfile")
        elif match.group(1) == "IanModuleEntityTable":
            contents.append("BoardInformation")
    ...
    for match in matches_init:
        ...
        for match1 in re.finditer(pattern_snmp, match.group(1)):
            ...
            if "ProfileIndex" in field_name or "profileIndex" in field_name:
                field_name = "index"
            field_name = field_name.replace(newKey, "")
            if field_name:
...
def tabel_column():
    new_data = {}
    new_data2 = {}
    file = open("new_java/data.json", 'r', encoding='UTF-8')
    data = json.load(file)
    for d in data:
        column = []
        column2 = []
    ...
    print(new_data)
    # # # 将更新后的表格数据写入新的 JSON 文件
    with open('new_java/column_data.json', 'w', encoding='utf-8') as j_file:
        json.dump(new_data, j_file, ensure_ascii=False, indent=4)
    with open('new_java/column_data2.json', 'w', encoding='utf-8') as j_file:
